...
def create_action(action_data: ActionCreate, db: Session = Depends(get_db), _: User = Depends(require_role(["admin"]))):
    if ActionService.get_action_by_name(db, action_data.action_name):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Action with this name already exists")
...
def delete_action(action_id: int, db: Session = Depends(get_db), _: User = Depends(require_role(["admin"]))):
    if not ActionService.delete_action(db, action_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Action not found")

...
def create_permission(permission_data: RolePermissionCreate, db: Session = Depends(get_db)):
...
def bulk_create_permissions(permissions_data: List[RolePermissionCreate], db: Session = Depends(get_db)):
...
def get_permissions_by_menu(menu_id: int, db: Session = Depends(get_db)):
...
def delete_permissions_by_role(role_id: int, db: Session = Depends(get_db)):

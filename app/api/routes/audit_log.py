...
def create_log(log_data: AuditLogCreate, db: Session = Depends(get_db), _: User = Depends(require_role(["admin"]))):
...
def get_all_logs(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return AuditLogService.get_all_logs(db, skip, limit)

@router.get("/user/{user_id}", response_model=List[AuditLogResponse])
def get_logs_by_user(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def get_logs_by_module(module: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
